...
### 参赛队员
- 第 $n(n\ge 1)$ 次晋级，需要 CF rating $X+2\times n$（根据 CF 规则，为了保证 rating 有效，需要至少参加过 $7$ 次 CF rated contest），根据实际情况 $X$ 可能会进行调整，一般为 $1600$，或 AT rating $Y+2\times n$，根据实际情况 $Y$ 可能会进行调整，一般为 $1400$（根据 AT 规则，没有场次要求）。
...
违反了「集训队的纪律」第二条或者对集训队造成不良影响的，将会被踢出集训队。若对集训队/学校造成恶劣影响，会上报教务处，根据教务处规定进行处罚，最严重会给予**开除学籍**处分。
